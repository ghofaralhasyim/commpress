...
<?= $this->section('content') ?>
   ...
   <div class="row">
    ...
    <div class="row mt-2">
        ...
        <div class="row pt-2">
            <div class="col">
                 <h5 class="mb-1 mt-1" style="font-weight: 400;"><i class="ri-chat-check-line"></i>List Pameran</h5>
            </div>
            <div class="col" style="text-align: right;">
                <button id="buttonShow" class="btn btn-primary transition-1" onclick="toggleForm()">Buat Pameran Baru</button>
            </div>
        </div>
        <form action="<?= base_url("/dashboard/pameran/new/save-pameran"); ?>" id="form" method="POST" 
        enctype="multipart/form-data" class="mt-2 mb-3 d-none" autocomplete="off">
            <div class="form-group">
                <label for="name">Name</label>
                <input type="text" class="form-control" id="name" name="name" placeholder="Nama pameran">
            </div>
        ...
        <div class="mt-2 pd-1">
            <table id="table" class="table table-striped" style="width:100%">
                <thead>
                 ...
                 <tbody>
                    <?php foreach ($listPameran as $pameran) : ?> 
                        <tr>
                            <td><?= $pameran->name ?></td>
                            <td><?= $pameran->participant ?></td>
                            <td><?= $pameran->status ?></td>
                            <td><a href="<?= base_url("/dashboard/pameran/{$pameran->slug}") ?>" >Details</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot>
